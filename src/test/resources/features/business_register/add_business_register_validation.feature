...
Feature: This feature is to verify the ability of the BR to detect and process invalid Add Business Register Notifications
  ...
  Scenario: I would like a BR-BusinessError with error code ERR_BR_0103 when the Receiver Country Code is not UK
    Given I am creating an AddBrNotification with details
      |  |  |  |  | AT |  |  |  |  |
    Then I should get a add Br Notification error message with the error code ERR_BR_0103
  ...
  Scenario: I would like a BR-BusinessError with error code ERR_BR_5102 returned if Register name is empty
    Given I am creating an AddBrNotification with details
      |  |  |  |  |  | EMPTY |  |  |  |
    Then I should get a add Br Notification error message with the error code ERR_BR_5102
